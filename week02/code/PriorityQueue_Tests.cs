...
{ 
    ...
    // Defect(s) Found: The priority comparison used >= instead of >, which caused an item later in the queue with the same priority to be selected before the item closest to the front.
    ...
    // Defect(s) Found: The PriorityQueue did not remove the item after returning it, so the same item could be returned again instead of moving to the next highest priority.
    ...
    // Defect(s) Found: None. The PriorityQueue correctly throws an InvalidOperationException with the required message.
